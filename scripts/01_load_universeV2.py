...
def main():
    ...
    # Previously: exchanges_to_load = config_v6.EXCHANGES_TO_TRACK
    # We now fetch the authoritative list from EODHD and intersect with our config to avoid 404s.
    try:
        supported = client.get_exchanges_list()  # returns list of dicts with at least {'Code': 'US', ...}
        supported_codes = {row.get('Code') for row in supported if row.get('Code')}
        configured = set(config_v6.EXCHANGES_TO_TRACK)

        # Keep only truly supported exchanges (by exact EODHD Code)
        exchanges_to_load = sorted(configured & supported_codes)

        # Warn about anything configured but not supported
        missing = sorted(configured - supported_codes)
        if missing:
            logger.warning(f"Skipping unsupported exchanges (not in EODHD list): {', '.join(missing)}")
    except Exception as e:
        logger.error(f"Failed to fetch exchanges list from EODHD: {e}")
        # Fall back to configured list, but this may 404 on bad codes
        exchanges_to_load = config_v6.EXCHANGES_TO_TRACK

    priority_exchanges = [ex for ex in config_v6.PRIORITY_EXCHANGES if ex in exchanges_to_load]
