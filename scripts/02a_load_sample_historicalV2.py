...
def process_batch(client: EODHDClient, engine, batch: List[dict]) -> dict:
    ...
    for i, asset in enumerate(batch, 1):
        ...
        if success and records > 0:
            ...
            if status == 'no_data':
                logger.info("  ⚠️  No data available from EODHD")
            elif status == 'paywall':
                logger.info("  🚧 Subscription tier required (402)")
            elif status == 'not_found':
                logger.info("  ❌ Symbol not found on EODHD (404)")
            else:
                logger.info(f"  ⚠️  Failed to load data (status: {status})")
            
            # Mark as inactive if loading failed
            with get_session(engine) as mark_session:
                try:
                    ...
                    if failed_asset:
                        ...
                        mark_session.commit()
                        logger.info(f"     (Marked as inactive)")
                except Exception as e:
                    logger.debug(f"Could not mark asset inactive: {e}")
